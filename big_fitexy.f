      SUBROUTINE big_fitexy(x,y,ndat,sigx,sigy,a,b,siga,sigb,chi2,q)
      INTEGER ndat,NMAX
      REAL x(ndat),y(ndat),sigx(ndat),sigy(ndat),a,b,siga,sigb,chi2,q,
     *POTN,PI,BIG,ACC
      PARAMETER (NMAX=10000,POTN=1.571000,BIG=1.e30,PI=3.14159265,
     *ACC=1.e-3)
CU    USES avevar,brent,big_chixy,fit,gammq,mnbrak,zbrent
      INTEGER j,nn
      REAL xx(NMAX),yy(NMAX),sx(NMAX),sy(NMAX),ww(NMAX),swap,amx,amn,
     *varx,vary,aa,offs,ang(6),ch(6),scale,bmn,bmx,d1,d2,r2,dum1,dum2,
     *dum3,dum4,dum5,brent,big_chixy,gammq,zbrent
      COMMON /fitxyc/ xx,yy,sx,sy,ww,aa,offs,nn
      EXTERNAL big_chixy
      if (ndat.gt.NMAX) pause 'NMAX too small in big_fitexy'
...
11    continue
      ...
      ang(6)=POTN
      do 12 j=4,6
        ch(j)=big_chixy(ang(j))
12    continue
      call mnbrak(ang(1),ang(2),ang(3),ch(1),ch(2),ch(3),big_chixy)
      chi2=brent(ang(1),ang(2),ang(3),big_chixy,ACC,b)
      chi2=big_chixy(b)
      a=aa
...
14    continue
      if (bmx.lt. BIG) then
        bmx=zbrent(big_chixy,b,b+bmx,ACC)-b
        amx=aa-a
        bmn=zbrent(big_chixy,b,b-bmn,ACC)-b
        amn=aa-a
